...
function ProgressTracker() {
  ...
  useEffect(()=>{
    const fetchPending = async()=>{
      try{
        let p_res = await fetch(`http://localhost:8000/submissions/pending`, {
          method: "GET",
          credentials:"include"
        })
        if(!p_res.ok){
    ...
    const fetchApproved = async()=>{
      try{
        let a_res = await fetch(`http://localhost:8000/submissions/approved`, {
          method: "GET",
          credentials:"include"
        })
        if(!a_res.ok){
    ...
    const fetchRejected = async()=>{
      try{
        let r_res = await fetch(`http://localhost:8000/submissions/rejected`, {
          method: "GET",
          credentials:"include"
        })
        if(!r_res.ok){
