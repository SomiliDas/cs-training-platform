...
export default function PendingApprovals() {
    ...
    useEffect(()=>{
        const getPendingApprovals = async ()=>{
            try{
                const res = await fetch("https://flight-training-platform-backend-cors.onrender.com/submissions/pending", {
                    method:"GET",
                    credentials:"include"
                })
                if(!res.ok){
    ...
    const acceptHandler = async(studentId , taskId)=>{
        try{
            const res = await fetch(`https://flight-training-platform-backend-cors.onrender.com/submissions/approve/${studentId}/${taskId}`, {
                method:"PUT",
                credentials:"include"
            })
            if(!res.ok){
    ...
    const rejectHandler = async(studentId, taskId)=>{
        try{
            const res = await fetch(`https://flight-training-platform-backend-cors.onrender.com/submissions/reject/${studentId}/${taskId}`, {
                method: "PUT",
                credentials : "include"
            })
            if(!res.ok){
