...
export default function PendingApprovals() {
    ...
    const acceptHandler = async(studentId , taskId)=>{
        try{
            const res = await fetch(`http://localhost:8000/submissions/approve/${studentId}/${taskId}`, {
                method:"PUT",
                credentials:"include"
            })
            if(!res.ok){
    ...
    const rejectHandler = async(studentId, taskId)=>{
        try{
            const res = await fetch(`http://localhost:8000/submissions/reject/${studentId}/${taskId}`, {
                method: "PUT",
                credentials : "include"
            })
            if(!res.ok){
